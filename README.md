...
## 開発工程<br>
要件定義→外部設計→内部設計→開発→単体テスト→結合テスト→システムテスト→運用テスト→システム移行

■要件定義
開発プロジェクトを進める前に、顧客にとって必要な機能や要件をまとめる作業
すべての要求を実現できるとは限らないため、ユーザーと話し合い要件に優先順位をつける

■外部設計（基本設計）
要件定義で作成した内容をもとに、外見的な見た目の部分を設計する
<---------方式設計--------->
・システムに使用するソフトウェア、アプリケーション
・開発に使用する言語やフレームワーク
<---------機能設計--------->
・画面遷移設計
開発しようとしているソフトウェアの全体の画面遷移図を記載する
・画面設計
画面を構成するレイアウト、各コンポーネントの表示仕様、入力仕様、操作仕様をなどを記載する
※実際の画面をイメージできて、操作方法が分かるように記載する
※画面のデザインを行う必要はない
※ワイヤーフレームとも呼ばれることがある
<--------その他設計-------->
